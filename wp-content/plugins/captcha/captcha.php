...
if ( ! function_exists ( 'cptch_init' ) ) {
	function cptch_init() {
		 ...
		 */
		if ( true ) {
			add_action( 'bp_before_registration_submit_buttons', 'cptch_buddypress_register_form' );
			//add_action( 'signup_extra_fields', 'wpmu_cptch_register_form' );
			//add_action( 'signup_blogform', 'wpmu_cptch_register_form' );
			...
			//		add_filter( 'wpmu_validate_user_signup', 'cptch_register_validate' );
			//}
			add_action( 'bp_signup_validate', 'cptch_bd_register_check' );			
		}
		/*
...
if ( ! function_exists( 'cptch_buddypress_register_form' ) ) {
	function cptch_buddypress_register_form() {
		echo '<div class="register-section">';
		echo cptch_display_captcha_custom( 'signup_form', 'cptch_wp_register' ) ;
		
		do_action( 'bp_recaptcha_response_errors' );  
		echo '</div>';
		return true;
	}
}
...
if ( ! function_exists ( 'cptch_bd_register_check' ) ) {
	function cptch_bd_register_check( $errors ) {
		global $bp;
		global $cptch_options;
		$str_key = $cptch_options['str_key']['key'];
		...
		if ( cptch_limit_exhausted() ) {
				...
				$bp->signup->errors['recaptcha_response'] = __( 'ERROR', 'captcha' ) . ':&nbsp;' . $cptch_options['time_limit_off'];
			 
		} elseif ( isset( $_REQUEST['cptch_number'] ) && "" == $_REQUEST['cptch_number'] ) {
			$bp->signup->errors['recaptcha_response'] = __( 'ERROR', 'captcha' ) . ':&nbsp;' . $cptch_options['no_answer'] ; 
			...
			$bp->signup->errors['recaptcha_response'] =__( 'ERROR', 'captcha' ) . ':&nbsp;' . $cptch_options['wrong_answer'] ;  
		}
		return false;
	}
}
