...
window.BuddyBoss_Media_Uploader = ( function ( window, $, util, undefined ) {
	...
	var APP = {
		...
		setup_textbox: function () {
			...
			_l.$whats_new.focus( function () {
				var bt = $('#browse-file');
				var dv = bt.parent().find('.moxie-shim');
				
				 
				 
				//dv.css("width","100%");
				dv.css("width",bt.css("width")); 
				 
				//dv.css("height","100%");
				dv.css("height",bt.css("height")); 
				dv.css("top",bt.position().top);
				dv.css("left",bt.position().left);
				dv.css("z-index","1");
			} );			
		},
		/**
		...
		start_uploader: function () {
			...
			if ( $( '#buddyboss-media-add-photo' ).length > 0  /*&& 'false' === BBOSS_MEDIA.is_photo_page */ ) {
				...
				input.onchange = function ( event ) {
					//_l.$open_uploder_button.click();//to open modal window
					uploader.addFile( event.target.files );
				};
