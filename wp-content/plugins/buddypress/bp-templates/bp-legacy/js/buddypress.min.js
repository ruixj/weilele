...
jq(document).ready( function() {
	...
	jq( '.dir-search, .groups-members-search' ).on( 'click', function(event) {
		...
		if ( target.attr('type') === 'submit' ) {
			el = jq('.dir-form .item-list-tabs li.selected');
			 
			if (el.length == 0)
			{
				el = jq('.dir-search input[type=hidden]');
				use_cookie = false;
			...
			else
			{
				...
				bp_filter_request( object, jq.cookie('bp-' + object + '-filter'), scope , 'div.' + object, target.parent().children('label').children('input').val(), 1, jq.cookie('bp-' + object + '-extras'), null, template );
	...
	jq('#buddypress').on( 'click', function(event) {
		...
		if ( target.parent().parent().hasClass('pagination') && !target.parent().parent().hasClass('no-ajax') ) {
            ...
            var bUseCookie = true;
			if ( jq('.dir-form .item-list-tabs li.selected').length ) {
				el = jq('.dir-form .item-list-tabs li.selected');
			} else {
			...
			{
				filter = "active";
				bp_filter_request( object,  filter , scope, 'div.' + object, search_terms, page_number, jq.cookie('bp-' + object + '-extras'), caller, template );
			}
			return false;
		}
