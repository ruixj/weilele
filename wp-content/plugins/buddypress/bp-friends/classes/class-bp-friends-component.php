		...
		if ( is_user_logged_in() ) {
			...
			if ( !empty( $count ) ) {
				$title = sprintf(
					...
					_x( '朋友 %s', 'My Account Friends menu', 'buddypress' ),
					'<span class="count">' . bp_core_number_format( $count ) . '</span>'
				);
				$pending = sprintf(
					/* translators: %s: Pending friend request count for the current user */
					_x( '好友请求 %s', 'My Account Friends menu sub nav', 'buddypress' ),
					'<span class="count">' . bp_core_number_format( $count ) . '</span>'
				);
			} else {
				$title   = _x( '朋友',            'My Account Friends menu',         'buddypress' );
			...
			$wp_admin_nav[] = array(
				...
				'id'       => 'my-account-' . $this->id . '-my-friends',
			...
			$wp_admin_nav[] = array(
				'parent'   => 'my-account-' . $this->id,
				'id'       => 'my-account-' . $this->id . '-membersearch',
				'title'    => _x( '添加朋友' , 'buddypress' ),
				'href'     => trailingslashit( $friends_link . 'membersearch' ),
				'position' => 30
			);		
